...
average_m = nan(length(T),t/interval-1);

parfor temp=1:length(T)
average_m_local = nan(1,t/interval-1);
sigma_old = ones(1,N);
sigma = sigma_old;
E_old = -J.*sum(sigma(1:end-1).*sigma(2:end))-H.*sum(sigma);
...
m_theoric = sinh(H./T)./sqrt(sinh(H./T).^2+exp(-4*J./T));

t_plot = interval.*(1:size(average_m,2));
figure
for i=1:length(T)
    plot(average_m(i,:),t_plot)
    hold on
end

figure
plot(T,average_m(:,end),'.')
hold on
plot(T,m_theoric,'.-')
